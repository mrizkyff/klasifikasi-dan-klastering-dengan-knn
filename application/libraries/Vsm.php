...
class VSM
{
    ...
    */
    ...
    /**
     * 
     * Perhitungan dice similarity
     * intinya perbedaannya dengan cosim adalah ini tu 2x(perkalian skalar) / (jumlah vektor d1 + jml vektor query)
     * @param array @bobot
     * @param $debug
     */
    public static function dice_similarity($dis, $debug)
    {
        ...
        foreach ($dokumenCosine as $index => $dokumen) {
            ...
            if ($dokumen["jumlah_vektor"] != 0 && $dokumen["jumlah_besar_vektor"] != 0) {
                // ganti rumus sesuai dengan aturan jaccard
                $jumlah = 2*($dokumen['jumlah_vektor']) / ($dokumen['jumlah_besar_vektor']);
            }
            $arrayDoc[$index] = ["id_doc" => $dokumen['id_doc'], "ranking" => $jumlah];
            array_push($dokumenRanking, $arrayDoc[$index]);
        }
